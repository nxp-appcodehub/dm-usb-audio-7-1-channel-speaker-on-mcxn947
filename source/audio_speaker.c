...
cs42448_config_t cs42448Config = {
    ...
    .i2cConfig = {.codecI2CInstance = BOARD_CODEC_I2C_INSTANCE, .codecI2CSourceClock = BOARD_CODEC_I2C_CLOCK_FREQ},
     ...
     (defined(USB_AUDIO_CHANNEL5_1) && (USB_AUDIO_CHANNEL5_1 > 0U)) || \
     (defined(USB_AUDIO_CHANNEL3_1) && (USB_AUDIO_CHANNEL3_1 > 0U)))
    .format = {.sampleRate = 48000U, .bitWidth = 24U, .mclk_HZ = 12288000},
    .bus    = kCS42448_BusTDM,
#else
    .format = {.sampleRate = 48000U, .bitWidth = 24U, .mclk_HZ = 12288000},
    .bus    = kCS42448_BusI2S, 
...
static void txCallback(hal_audio_handle_t handle, hal_audio_status_t completionStatus, void *callbackParam)
{
    ...
    else
    {
     ...
     (defined(USB_AUDIO_CHANNEL5_1) && (USB_AUDIO_CHANNEL5_1 > 0U)) || \
     (defined(USB_AUDIO_CHANNEL3_1) && (USB_AUDIO_CHANNEL3_1 > 0U)))
            xfer.dataSize = (HS_ISO_OUT_ENDP_PACKET_SIZE / AUDIO_FORMAT_SIZE * 4U) / AUDIO_FORMAT_CHANNELS *
                                    DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT;
#else
            xfer.dataSize = AUDIO_PLAY_BUFFER_SIZE_ONE_FRAME;
#endif
        }
...
void AUDIO_DMA_EDMA_Start()
{
    ...
    hal_audio_transfer_t xfer   = {0};
    mclkConfig.mclkOutputEnable = true, mclkConfig.mclkHz = 12288000;
    mclkConfig.mclkSourceClkHz = 12288000;
    ...
    xfer.dataSize = (HS_ISO_OUT_ENDP_PACKET_SIZE / AUDIO_FORMAT_SIZE * 4U) / AUDIO_FORMAT_CHANNELS *
                            DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT;
#else
    xfer.dataSize = HS_ISO_OUT_ENDP_PACKET_SIZE;
#endif
    xfer.data     = audioPlayDMATempBuff;
...
void APPInit(void)
{
    ...
    /* InputMUX */
    // 102- SAI1 TX DMA Request
    INPUTMUX0->DMA0_REQ_ENABLE3 |= 1 << INPUTMUX_DMA0_REQ_ENABLE3_REQ102_EN0_SHIFT;
    INPUTMUX0->DMA0_REQ_ENABLE3 |= 1 << INPUTMUX_DMA0_REQ_ENABLE3_REQ101_EN0_SHIFT;
...
#endif
{
    ...
    GPIO_PinInit(GPIO0, 29, &gpio_config);
   
    GPIO_PinInit(DEMO_CODEC_RESET_GPIO, DEMO_CODEC_RESET_GPIO_PIN, &gpio_config);
    
    //Enable Audio DC Power
    GPIO_PinInit(DEMO_DC_ENABLE_GPIO, DEMO_DC_ENABLE_GPIO_PIN, &gpio_config);
    GPIO_PinWrite(DEMO_DC_ENABLE_GPIO, DEMO_DC_ENABLE_GPIO_PIN, 1U);
